...
| [flutter-tizen-create-plugin](skills/flutter-tizen-create-plugin/SKILL.md) | Scaffold a C++ Tizen plugin, wire a method channel, link Tizen Native libraries, declare privileges, validate via the example app. Use when no existing plugin wraps a needed Tizen Native API. | Wrap the Tizen `system_info` API as a new `system_info_tizen` plugin. |
...
- "privilege", "tizen-manifest", "messageport", "app control" → `flutter-tizen-use-plugins`
- "new tizen plugin", "C++ Tizen plugin", "method channel" → `flutter-tizen-create-plugin`
